...
import { DataStore, Quizzes, Session, getData, state, Questions, Answer, player } from './dataStore';
import { getNow, findAtQuestionMetadata, findQuizSession } from './helper';
...
function calculateAnswerTime(session: Session) {
...
function calculateScore(session: Session, question: Questions, atQuestion: questionResults): number {
  ...
  return Math.round(questionPoints / playerNameInde);
}
